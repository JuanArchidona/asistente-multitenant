...
| R-20 | Salida insegura: la respuesta se ejecuta en algún sitio | CC | 2 | — | B | `agent.py` | La salida es texto para una persona; no alimenta ningún intérprete ni acción | **Por construcción.** Con WhatsApp y correo la salida viaja por un canal que la puede reenviar; sigue siendo texto y el registro no guarda la respuesta |
...
| R-24 | Suplantación del remitente en el canal de correo: un `From` de una dirección autorizada enviado desde otro sitio | CC | 1 | AML.T0051.000 | B | `canal_correo.py`, `tests/test_canal_correo.py` | El canal exige `Authentication-Results` con `dkim=pass` o `spf=pass`, cabecera que escribe el servidor receptor y no el remitente; sin ella el correo se rechaza con frase fija y se anota por huella, y no llega al modelo | **Por construcción, probado en simulación.** Cuatro pruebas fijan el rechazo; la prueba en vivo (correo con `From` falsificado desde un servidor sin DKIM) está en la lista de `CANAL_CORREO.md` y espera el buzón. Residual: depende de que el proveedor del buzón escriba la cabecera; Gmail lo hace siempre |

## 3. Vista por cuadrante de Rumsfeld

| Cuadrante | Contramedida que pide el 4.4 | Filas | Lo que el proyecto pone |
|---|---|---|---|
| Conocidos-conocidos | Pruebas y métricas | R-01, R-02, R-03, R-05, R-08, R-10, R-12, R-14, R-16, R-17, R-18, R-19, R-20, R-24 | Los bancos, 122 casos en tres inquilinos, las métricas deterministas, la contabilidad de coste |
