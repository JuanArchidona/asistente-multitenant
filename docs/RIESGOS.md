...
| R-02 | Divulgación de información confidencial a un usuario sin permiso | CC | 6 | AML.T0057 | A | §2, §5, §8, §9, §11, §22 | Permiso **dentro del `where`** de la búsqueda y redacción de campos al salir de la herramienta; el modelo nunca ve lo que no puede decir | **Medido.** Cobertura del riesgo 0,636 (A) y 1,0 (C); `fuga_literal` 10/10; una fuga real (§5) medida y corregida |
