...
| Conmutación de proveedor a Gemini | Arreglada: era una forma y no un hecho; verificada de extremo a extremo (§29). **Comparar el generador con Gemini quedó bloqueado el 26-09** (§54): la clave es del nivel gratuito, 20 peticiones al día, y 73 de 82 casos murieron en 429; exige facturación en Google |
| Canales (correo, WhatsApp) | **WhatsApp construido** el 23-09 (`src/canal_whatsapp.py`, `docs/CANAL_WHATSAPP.md`): el número es la credencial y fija el inquilino, artículo 50 en la primera conversación, aprobación humana por texto, firma del webhook obligatoria, teléfono nunca en el registro; 25 pruebas con mensajes simulados y tres mensajes simulados contra el sistema real (5,1 s una consulta). **Conectado en vivo el 24-09** por la app de Claude con Juan (E-0009, §51): app de Meta publicada, webhook verificado, mensajes reales llegando; las dos primeras consultas dieron `NotFoundError` porque el servidor no construía el índice en el disco efímero de Render, corregido el mismo día. **Probado en vivo tras el redespliegue** (12:03): la pregunta de vacaciones responde como en la web citando el convenio, y la del salario deniega sin el dato y sin fuga, con la línea "Retenido por permiso"; menos de un minuto por pregunta a precisión de minuto. **Token permanente el 25-09** (E-0010): usuario del sistema de Meta, caducidad *Nunca* según el depurador, despliegue de 1 min 35 s y las dos preguntas de control respondidas igual a las 07:32 y 07:33; 18 min desde el encargo. **Pruebas en vivo restantes canceladas el 26-09** (aislamiento con dos números, aprobación por texto): mismo mecanismo que la credencial de la web, cubierto por las pruebas simuladas. **Correo construido el 26-09** (`src/canal_correo.py`, `src/canal_correo_servidor.py`, `docs/CANAL_CORREO.md`): la dirección es la credencial, se exige DKIM/SPF en `pass` (R-24), aprobación en la primera línea, sondeo IMAP y respuesta SMTP enhebrada, sin paquetes nuevos; 34 pruebas y una consulta simulada contra el sistema real en 5,26 s. **Pendiente de conectar** a un buzón de Gmail dedicado que crea Juan (`docs/ENCARGO_CORREO_2026-09-26.md`); servicio `asistente-correo` en el blueprint, que falla a propósito hasta tener credenciales. Límite declarado: dormido en el plan gratuito no sondea |
...
| Memoria técnica (bloque 4, punto 16) | **Borrador 1 escrito** el 24-09: `docs/MEMORIA.md`, estructura según las cinco características del capstone del programa, cada cifra con su hallazgo y ejecución, contrastada contra los hallazgos (54 el 26-09). Segunda pasada el mismo día: ficha de coste mensual por inquilino (5.4), capítulo de documentación y defensa (hoy el 10), revisión completa; 23 páginas con `docs/entrega/construir_pdf.py --entrada docs/MEMORIA.md`. El capítulo 3.1 pasó de argumentado a medido con el §50. Rotar claves y cronometrar la mitad manual del simulacro: **decidido en contra** por Juan el 24-09. WhatsApp probado en vivo el 24-09 (§51). **Borrador 2 el 26-09** tras una segunda lectura hostil (7 altas, 20 medias, 8 bajas; informe en `docs/REVISION_MEMORIA_2026-09-26.md`, aplicado salvo la lectura de la latencia interna de WhatsApp en Render): la pila justificada por criterio, la ficha de coste con el plan de pago de Render (7 USD/mes por servicio), la fuga por hora derivada del §53 y el anexo de hallazgos regenerado desde las citas reales; 30 páginas. **Lectura lineal el mismo día** (`docs/LECTURA_MEMORIA_2026-09-26.md`): caja de definiciones en 1.3, once contradicciones de fondo cerradas (mayoría de tres, conmutación como contingencia, registro en disco efímero), WhatsApp en cinco líneas, capítulos reordenados para terminar en las conclusiones, 8.3 y 9 sin duplicados. Queda un hueco marcado `[PENDIENTE]`: el formato de la defensa; la rúbrica puede reordenar capítulos pero no cambia las cifras |
...
docs/
  ...
  CANAL_CORREO.md    Canal de correo: dirección como credencial, DKIM/SPF exigido, puesta en marcha, límites
...
src/canal_correo.py, src/canal_correo_servidor.py       Canal de correo (lógica pura + sondeo IMAP y respuesta SMTP con la librería estándar)
...
uv run python -m src.canal_correo_servidor --simular "..." --desde empleado@ejemplo.es   # canal de correo en local, sin buzón
...
uv run python scripts/prueba_carga.py --remoto         # puerta HTTP de Render, coste cero (§53)
uv run python scripts/prueba_carga.py --local --agencia --etiqueta X   # pipeline bajo concurrencia, ~0,12 USD (§53)
